...
export function TagNetworkGraphClient({
    ...
    const tagNodesOnly = nodes.filter((n) => n.type === 'tag');
    const documentNodesOnly = nodes.filter((n) => n.type === 'document');

    const tagIndexMap = new Map(
        tagNodesOnly.map((node, idx) => [node.id, idx] as const)
    );
    const documentIndexMap = new Map(
        documentNodesOnly.map((node, idx) => [node.id, idx] as const)
    );

    const tagCount = tagNodesOnly.length || 1;
    const documentCount = documentNodesOnly.length || 1;

    // Position nodes
    const positionedNodes = nodes.map((node) => {
        if (node.type === 'tag') {
            // Tags in inner circle
            const tagIndex = tagIndexMap.get(node.id) ?? 0;
            ...
            const docIndex = documentIndexMap.get(node.id) ?? 0;
            const angle = (docIndex / documentCount) * 2 * Math.PI;
            const radius = 150;
            return {
                ...node,
                x: centerX + radius * Math.cos(angle),
